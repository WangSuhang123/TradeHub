...
<div class="min-h-screen bg-gray-100 py-8">
  <div class="max-w-2xl mx-auto px-4">
    <div class="bg-white rounded-2xl shadow-lg p-8">
      <h1 class="text-2xl font-bold text-gray-800 mb-6">编辑商品</h1>
      ...
      <form action="${pageContext.request.contextPath}/product" method="post" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select name="condition_level" required
              ...
              <option value="3" ${product.conditionLevel == 3 ? 'selected' : ''}>有使用痕迹</option>
              <option value="4" ${product.conditionLevel == 4 ? 'selected' : ''}>老旧</option>
            </select>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="relative">
              <span class="absolute left-3 top-3 text-gray-500">¥</span>
              <input type="number" name="price_original" value="${product.priceOriginal > 0 ? product.priceOriginal : ''}" step="0.01" min="0"
                     class="w-full pl-8 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name="description" rows="5"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">${product.description}</textarea>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">商品图片</label>
          <c:if test="${not empty product.image}">
            <img src="${pageContext.request.contextPath}/${product.image}" class="w-32 h-32 object-cover rounded-lg mb-2">
          </c:if>
          <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
            <input type="file" name="image" accept="image/*"
                   class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
            <p class="text-xs text-gray-400 mt-2">留空则不修改图片，支持 JPG、PNG、GIF 格式</p>
          </div>
        </div>
